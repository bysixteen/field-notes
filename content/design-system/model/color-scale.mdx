---
title: Colour Scale
description: The 12-step colour ramp (50–1000) and the UI role of each step.
---

## Definition

Every sentiment palette is a **12-step ramp** numbered from 50 to 1000. The numbering follows a Radix-style convention where lower steps are subtler (backgrounds) and higher steps are bolder (text, solid fills). Each step has a defined role in the UI so that palettes remain consistent regardless of the underlying hue.
...
Colour-scale steps are **primitives** — they should never appear directly in component styles. Instead, semantic tokens reference specific steps and are the values components consume.
