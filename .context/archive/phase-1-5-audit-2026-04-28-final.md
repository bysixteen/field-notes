...
**Worktree:** `<live worktree>` (was `paris-v4` at the time of writing)
**Plugin artefact root:** `<workspace>/dist/` (workspace, not worktree)
...
| Honest deferral | 0 | 1 (Phase 3 consumer calibration) | +1 |
...
| **1.3** SELF-REVIEW (#104) | #145 | FAIL (`SELF-REVIEW.md` missing; no skill invokes it) | **PASS (with note)** | Y | `.claude/skills/_foundations/SELF-REVIEW.md` exists (1943 bytes). `grep -L "SELF-REVIEW\|self-review" .claude/skills/fn-*.md .claude/skills/fn-*/SKILL.md .claude/skills/start-*/SKILL.md` returns only `.claude/skills/fn-design/SKILL.md` — a 14-line alias delegating to `fn-design-md/SKILL.md`, which itself invokes the protocol at line 211. Disagreement with kathmandu-v1's strict literal-grep recipe is noted explicitly per the "do not silently broaden the rubric" rule; tracked as follow-up **#153**. |
| **2.2** fn-init in plugin | #141 | FAIL (`unzip -l` showed 0 `skills/fn-init/`) | **PASS** | Y | `<workspace>/dist/field-notes-toolkit.plugin` exists at workspace root (162235 bytes, mtime 2026-04-28 09:57); `unzip -l \| grep "skills/fn-init/" \| wc -l` → **19** entries |
| **3** consumer calibration | n/a (human design-judgment work) | UNKNOWN (the `1 UNKNOWN` in kathmandu-v1's totals) | **HONEST DEFERRAL** | n (no flip; methodological reclassification only) | Out of Phase 1–5 scope per `FIX-MISSING-ELEMENTS.md` line 22 + ROADMAP. Scheduled separately by Daniel as design judgment. NOT scored FAIL. |
| **4.1** README (#120) | #143 | FAIL (README pre-Phase-1) | **PASS** | Y | All 5 brief-required tokens match in `README.md`: `from-dimensional` (3), `migrate` (3), `field-notes-toolkit\.plugin` (2), `fn-init` (4), `preservation marker` (1) |
...
5. **Phase 1.3 row scored PASS-with-note** (not FAIL) — `fn-design/SKILL.md` is a 14-line alias pointer that delegates to `fn-design-md/SKILL.md`. Canonical `fn-design-md/SKILL.md:211` does invoke `_foundations/SELF-REVIEW.md`. Methodologically consistent with addis-ababa row 1.1 (PASS-with-note when the literal verification recipe is stale due to a refactor — here, the alias creation absorbs the recurring `fn-design`-vs-`fn-design-md` confusion). Disagreement with kathmandu-v1's strict literal-grep recipe is noted explicitly per the "do not silently broaden the rubric" rule. **Tracked as follow-up #153** ("fn-design alias: SELF-REVIEW invocation policy", `defer`-labelled).
...
- **#153** — `fn-design alias: SELF-REVIEW invocation policy`
