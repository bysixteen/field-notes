# Bootstrap Colour Architecture

> Create the colour ramp generator seeded with user-provided anchor values.

## Required Inputs

| Input | Type | Description |
|-------|------|-------------|
| Anchor colours | Record | Map of colour name → OKLCH anchor value (e.g., `{ "blue": "oklch(0.55 0.24 260)" }`) |

## Expected Outputs

- `packages/ui/generate-ramps.ts` containing the anchor map and ramp generation logic.
- Generated 12-step colour ramps for each anchor when the script is executed.
...
1. Ask the user for their anchor colours — a set of name/OKLCH pairs.
2. Create `packages/ui/generate-ramps.ts`:
   - Define an `ANCHORS` constant with the user-provided colour map.
...
5. Confirm the output contains 12 steps per anchor colour.
...
- Each ramp step is a valid OKLCH colour string.
