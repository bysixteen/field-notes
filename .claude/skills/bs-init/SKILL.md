...
description: >-
  ...
  `bs-design-md --from-dimensional` expects, so a freshly bootstrapped project
  can immediately regenerate a valid `DESIGN.md`.
---

# bs-init — bootstrap canonical context stack
...
`bs-init` scaffolds the canonical four-piece context stack a downstream project
needs to consume `bs-design-md --from-dimensional`:
...
| `DESIGN.md` | Empty preservation markers in place (`<!-- bs-design-md:generated:start -->` ... `<!-- bs-design-md:generated:end -->`) so the first `bs-design-md` run fills the generated portion and any prose written after the `:end` marker is preserved across regenerations. |
...
- `--force` — required if `<dir>` is non-empty. Without it, `bs-init` refuses to overwrite.
- `--help` / `-h` — usage.

## Round-trip verification

After `bs-init`, the scaffold is ready to feed `bs-design-md --from-dimensional`:

```sh
node .claude/skills/bs-init/scripts/init.mjs --name test-project --target /tmp/test-project
node .claude/skills/bs-design-md/scripts/emit-design-md.mjs --from-dimensional /tmp/test-project --out /tmp/test-project --name test-project
```
...
- `bs-init` does not run `bs-design-md`. The two skills compose; running them in order is the bootstrap recipe.
