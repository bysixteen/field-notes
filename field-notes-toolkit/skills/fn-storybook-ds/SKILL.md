...
|-------|---------|-------|------|
| 0 | Load helper API | `fn-storybook-helpers` | Always (first) |
| 1 | Component documentation | `fn-storybook-docs` | Target is a component |
| 2 | Foundation documentation | `fn-storybook-foundations` | Target is tokens/scale |
...
2. **Load helper API (stage 0).** Always run `fn-storybook-helpers` first. This loads the complete catalogue of documentation helper components. All subsequent stages must use exclusively these helpers.
...
3. Generate the equivalent Figma documentation page (when `fn-figma-docs` is available and Prism MCP is connected)
