...
The Google `DESIGN.md` spec covers `Overview`, `Colors`, `Typography`, `Components`, and `Do's and Don'ts`. These are the boilerplate sections the emitter generates inside the marker block. The spec does **not** cover the full surface a consumer needs to brief a designer or an agent on a specific product — that surface lives in hand-authored prose appended below `<!-- bs-design-md:generated:end -->`.
