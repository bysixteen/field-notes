...
|---------------|---------------------|
| `fn-css` | CSS consumes tokens -- audit CSS alongside token integrity |
| `fn-component-api` | Dimension inputs (sentiment, emphasis, size) map 1:1 to token collections |
| `fn-component-scaffold` | Scaffold sets up initial token consumption in CSS |
| `fn-review` | Full review runs token audit as Stage 4 |
| `fn-storybook-foundations` | Foundation stories document the token system visually |
| `fn-storybook-docs` | Component docs include a TokenReference story |
